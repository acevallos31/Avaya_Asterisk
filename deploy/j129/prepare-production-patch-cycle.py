...
MARKER = '# J129-PRODUCTION-PATCH-CYCLE-V010'

text = HELPER.read_text()
if MARKER in text:
    print('PRODUCTION-PATCH-PREPARE-ALREADY-PRESENT')
    sys.exit(0)
...
# J129-PRODUCTION-PATCH-CYCLE-V010
production_patch_cycle_v010() {
  local repo_root="$1"
  local installer="$repo_root/dist/j129-v0.1.0/install.sh"
  ...
  echo "PATCH_BASELINE_FILES vendor=$before_vendor j129=$before_j129 global=$before_global http=$before_http"
  echo "PATCH_BASELINE_DB max=$before_max sip=$before_sip iax=$before_iax"

  bash "$installer" preflight
  bash "$installer" install
  bash "$installer" verify
  echo 'PATCH-FIRST-INSTALL-PASS'

  # Segunda instalación: debe ser idempotente y conservar el backup original.
  bash "$installer" install
  bash "$installer" verify
  echo 'PATCH-SECOND-INSTALL-IDEMPOTENT-PASS'
  ...
  [ "$after_vendor" = "$before_vendor" ] || { echo 'ERROR: rollback no restauro Avaya.py' >&2; exit 1; }
  [ "$after_j129" = "$before_j129" ] || { echo 'ERROR: rollback no restauro Avaya_J129.tpl' >&2; exit 1; }
  [ "$after_global" = "$before_global" ] || { echo 'ERROR: rollback no restauro Avaya_global_SIP.tpl' >&2; exit 1; }
  [ "$after_http" = "$before_http" ] || { echo 'ERROR: rollback no restauro Apache conf' >&2; exit 1; }
  [ "$after_max" = "$before_max" ] || { echo 'ERROR: rollback no restauro max_accounts' >&2; exit 1; }
  [ "$after_sip" = "$before_sip" ] || { echo 'ERROR: rollback no restauro max_sip_accounts' >&2; exit 1; }
  [ "$after_iax" = "$before_iax" ] || { echo 'ERROR: rollback no restauro max_iax2_accounts' >&2; exit 1; }

  echo 'PATCH-ROLLBACK-EXACT-PASS'
  echo 'J129-PRODUCTION-PATCH-CYCLE-PASS'
}
'''

text = text.replace(case_marker, func + '\n' + case_marker, 1)
dispatch = case_marker + '  production-patch-cycle-v010) [ -n "$OVERLAY_ROOT" ] || usage; production_patch_cycle_v010 "$OVERLAY_ROOT" ;;\n'
text = text.replace(case_marker, dispatch, 1)
HELPER.write_text(text)
print('PRODUCTION-PATCH-PREPARE-PASS')
