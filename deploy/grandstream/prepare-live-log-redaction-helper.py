...
inspect_grandstream_sensitive_logging() {
  local unsafe1 unsafe2 unsafe_count redaction_marker_count
  [ -f "$GRANDSTREAM_PY" ] || { echo "ERROR: no existe $GRANDSTREAM_PY" >&2; exit 1; }
  unsafe1="$(grep -Fc 'jsonvars vacio %s@%s GXP140x - vars rejected by interface - %s - %s - %s' "$GRANDSTREAM_PY" || true)"
  unsafe2="$(grep -Fc 'Endpoint %s@%s GXP140x - vars rejected by interface - %s - %s - %s' "$GRANDSTREAM_PY" || true)"
  unsafe_count=$((unsafe1 + unsafe2))
...
apply_grandstream_log_redaction() {
  local backup="$STATE_DIR/Grandstream.py.pre-g10-19h8f0" tmp before_sha after_sha unsafe1 unsafe2 unsafe_before unsafe_after
  [ -f "$GRANDSTREAM_PY" ] || { echo "ERROR: no existe $GRANDSTREAM_PY" >&2; exit 1; }
  install -d -m 0755 "$STATE_DIR"

  unsafe1="$(grep -Fc 'jsonvars vacio %s@%s GXP140x - vars rejected by interface - %s - %s - %s' "$GRANDSTREAM_PY" || true)"
  unsafe2="$(grep -Fc 'Endpoint %s@%s GXP140x - vars rejected by interface - %s - %s - %s' "$GRANDSTREAM_PY" || true)"
  unsafe_before=$((unsafe1 + unsafe2))
  ...
  [ "$unsafe1" -eq 1 ] && [ "$unsafe2" -eq 1 ] || { echo "ERROR: unexpected unsafe logging signature counts first=$unsafe1 second=$unsafe2" >&2; exit 1; }
...
PY
  ...
  unsafe1="$(grep -Fc 'jsonvars vacio %s@%s GXP140x - vars rejected by interface - %s - %s - %s' "$tmp" || true)"
  unsafe2="$(grep -Fc 'Endpoint %s@%s GXP140x - vars rejected by interface - %s - %s - %s' "$tmp" || true)"
  unsafe_after=$((unsafe1 + unsafe2))
