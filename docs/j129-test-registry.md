...
| 68 | `lab-endpoint-credential-test68.yml` | `68 | Issabel Lab | Endpoint Credentials | Runtime Smoke` | `RETRY-PENDING`: run `34683934974` instaló esquema/runtime y completó el smoke autenticado `FACTORY` PASS; falló solo la verificación final por invocar directamente un CLI fuera de sudoers. Corregido mediante acción sanitizada del helper allowlisted, sin ampliar permisos. No cambia el teléfono ni ejecuta Configure. |
