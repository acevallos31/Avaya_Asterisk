...
| 64 | `lab-grandstream-grp2601p-auth-read.yml` | `64 | Endpoint Lab | GRP2601P | Authenticated Provisioning Read` | `CREDENTIAL-BLOCKED` run `34620889592`: server preflight PASS y fila exacta confirmada; el secret `GRANDSTREAM_GRP_HTTP_DEFAULT_PASSWORD` no existe, por lo que no se intentó login ni otra credencial. Workflow verde por bloqueo esperado y sin escrituras. |
