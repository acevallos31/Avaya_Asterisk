...
| 55 | `lab-grandstream-test55-factory-reset.yml` | `55 | Issabel Lab | GXP1625 Factory Reset | Controlled Test` | FACTORY RESET + CONFIGURACIÓN SERVER-SIDE PASS / VALIDACIÓN FÍSICA PENDIENTE. Run `34541419730`: reset exacto observado (HTTP down/up); el endpoint reapareció por discovery. Run `34542431592`: baseline limpio en `.168`. Run `34543757258`: Configure manual corroborado, asociación 202, cfg/XML ligados a MAC y Ashly, HTTP 200, SIP 202 en `.168`; 201 fuera de esa IP. Push solo audita; repetir reset exige dispatch y confirmación exacta. Pendiente llamada/audio física. |
