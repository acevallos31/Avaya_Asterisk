...
| 64 | `lab-grandstream-grp2601p-auth-read.yml` | `64 | Endpoint Lab | GRP2601P | Authenticated Provisioning Read` | `CREDENTIAL-BLOCKED` run `34620889592`. Run `34628195632`: `HARNESS-FAIL` de parser después de recibir respuesta. Run definitivo `34628373795`: server preflight e identidad exacta PASS; candidato único `GRANDSTREAM_GXP_HTTP_DEFAULT_PASSWORD` rechazado (`login=FAILED`, HTTP 200, `GXP-CANDIDATE-REJECTED`). Sin escrituras, reinicio, provisioning ni firmware upgrade. Pendiente contraseña de etiqueta en secret GRP dedicado. |
