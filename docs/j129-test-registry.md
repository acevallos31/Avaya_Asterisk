...
| 49 | `lab-j129-physical-call-e2e.yml` | `49 | Issabel Lab | J129 Physical Call | Controlled E2E` | PREPARADA. Réplica LAB de Test 47 con helper restringido. Obtiene la cuenta J129 desde Endpoint Configurator (o valida una extensión opcional), resuelve dinámicamente la IP desde el peer SIP registrado en Asterisk y origina `Playback hello-world`. La IP manual ya no forma parte del contrato normal de la prueba. Pendiente reinstalar helper actualizado y ejecutar preflight/call con confirmación física. |
