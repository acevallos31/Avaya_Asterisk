...
| 64 | `lab-grandstream-grp2601p-auth-read.yml` | `64 | Endpoint Lab | GRP2601P | Authenticated Provisioning Read` | `CREDENTIAL-BLOCKED` run `34620889592`. Runs `34628195632`/`34628373795`: prueba no autoritativa con el secret equivocado `GRANDSTREAM_GXP_HTTP_DEFAULT_PASSWORD`; incluyó corrección del parser. Run autoritativo `34642689024`: server preflight e identidad exacta PASS; `GRANDSTREAM_GXP1625_HTTP_PASSWORD` presente pero rechazado (`login=FAILED`, HTTP 200, `GXP-CANDIDATE-REJECTED`). Sin escrituras, reinicio, provisioning ni firmware upgrade. Pendiente contraseña de etiqueta en secret GRP dedicado. |
