...
| 68 | `lab-endpoint-credential-test68.yml` | `68 | Issabel Lab | Endpoint Credentials | Runtime Smoke` | `INFRA-BLOCKED` run `34683238936`: static/guard/helper PASS; MySQL 1142 antes de instalar por grant DDL retirado. Tras `CREATE, REFERENCES` temporal, instala esquema/clave/UI persistentes, guarda override cifrado del GRP2601P por stdin, valida login read-only y marca `VALIDATED`. No cambia el teléfono ni ejecuta Configure. |
