...
| 67 | `tests/audit/test_endpoint_credential_foundation.py` (en implementación) | `67 | Repository | Endpoint Credential Foundation | Static Security Audit` | EN IMPLEMENTACIÓN: esquema reversible, clave externa, bóveda AES-256-GCM, pantalla global, guard CSRF y rotación `PENDING` sin aplicar endpoints. Pendiente ejecución en LAB, override por MAC, importación, rotación/rollback completa y consumo del vendor. |
