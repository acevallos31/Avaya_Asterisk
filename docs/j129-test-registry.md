...
| 68 | `lab-endpoint-credential-test68.yml` | `68 | Issabel Lab | Endpoint Credentials | Runtime Smoke` | `RETRY-PENDING` run `34683238936`: grant permanente por tabla resuelto; esquema y runtime persistentes PASS. El segundo intento detectó antes del acceso al teléfono que la contraseña de etiqueta debe almacenarse como `FACTORY`, no como override final. Corrección preparada para repetir login read-only y marcar `VALIDATED`; no cambia el teléfono ni ejecuta Configure. |
