...
V1_MARKER = 'CEIBA-GRANDSTREAM-PROD-V1'
V2_MARKER = 'CEIBA-GRANDSTREAM-PROD-V2-GXP16XX'
...
# Port only the final LAB-validated native GXP16xx HTTP session contract.
gxp_start = text.index(method_anchor)
gxp_next = text.find("\n    def ", gxp_start + len(method_anchor))
gxp_end = len(text) if gxp_next < 0 else gxp_next
prefix, gxp_body, suffix = text[:gxp_start], text[gxp_start:gxp_end], text[gxp_end:]

old_headers = "            headers = {'Content-Type': 'application/x-www-form-urlencoded'}\n"
new_headers = """            headers = {\n                'Content-Type': 'application/x-www-form-urlencoded',\n                # G10-19H8J-FINAL-NATIVE-GXP16XX / CEIBA-GRANDSTREAM-PROD-V2-GXP16XX\n                'Accept': '*/*',\n                'Host': self._ip,\n                'Referer': 'http://%s/' % self._ip,\n            }\n"""
if gxp_body.count(old_headers) != 1:
    raise SystemExit('GXP16xx headers anchor mismatch')
gxp_body = gxp_body.replace(old_headers, new_headers, 1)

old_sid = "            sid = jsonvars['body']['sid']\n"
new_sid = """            sid = jsonvars['body']['sid']\n\n            # G10-19H8J-FINAL-NATIVE-GXP16XX / CEIBA-GRANDSTREAM-PROD-V2-GXP16XX\n            # Preserve dologin cookies and reproduce the browser session identity.\n            cookie_parts = []\n            for header_name, header_value in response.getheaders():\n                if header_name.lower() == 'set-cookie':\n                    cookie_pair = header_value.split(';', 1)[0].strip()\n                    if cookie_pair and not cookie_pair.lower().startswith('session-identity='):\n                        cookie_parts.append(cookie_pair)\n            cookie_parts.append('session-identity=' + sid)\n            headers['Cookie'] = '; '.join(cookie_parts)\n"""
if gxp_body.count(old_sid) != 1:
    raise SystemExit('GXP16xx SID anchor mismatch')
gxp_body = gxp_body.replace(old_sid, new_sid, 1)
text = prefix + gxp_body + suffix
