...
make_defaults(){
  ...
  [ -n "$h" ] || h=localhost
  f="$(mktemp /tmp/endpoint-config-rc3-db.XXXXXX.cnf)"; chmod 0600 "$f"
  {
    echo '[client]'
    printf 'user="%s"\n' "$(escape_mysql "$u")"
    printf 'password="%s"\n' "$(escape_mysql "$p")"
...
bootstrap_schema_with_dba() (
  ...
  dba_cnf="$(mktemp /tmp/endpoint-config-rc3-dba.XXXXXX.cnf)"
  chmod 0600 "$dba_cnf"
...
VALUES
  (NULL, 'INSTALL_PRIVILEGE_TEST', 'PENDING', 'rc3-installer',
   '00000000-0000-0000-0000-000000000003', NOW());
UPDATE endpoint_credential_event
SET result='PASS'
WHERE correlation_id='00000000-0000-0000-0000-000000000003';
...
install_all(){
  ...
  local installing=1
  trap 'rc=$?; if [ "$installing" -eq 1 ]; then echo "[Endpoint Configuration 1.0.1-rc3] ERROR: restaurando runtime desde backup RC2" >&2; restore_runtime || true; apachectl -t >/dev/null 2>&1 && systemctl reload httpd || true; fi; exit "$rc"' ERR
