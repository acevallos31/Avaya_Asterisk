...
make_defaults(){
  ...
  [ -n "$h" ] || h=localhost
  f="$(mktemp /tmp/endpoint-config-rc4-db.XXXXXX.cnf)"; chmod 0600 "$f"
  {
    echo '[client]'
    printf 'user="%s"\n' "$(escape_mysql "$u")"
    printf 'password="%s"\n' "$(escape_mysql "$p")"
...
bootstrap_schema_with_dba() (
  ...
  dba_cnf="$(mktemp /tmp/endpoint-config-rc4-dba.XXXXXX.cnf)"
  chmod 0600 "$dba_cnf"
...
VALUES
  (NULL, 'INSTALL_PRIVILEGE_TEST', 'PENDING', 'rc4-installer',
   '00000000-0000-0000-0000-000000000004', NOW());
UPDATE endpoint_credential_event
SET result='PASS'
WHERE correlation_id='00000000-0000-0000-0000-000000000004';
...
check_payload(){
  ...
  check_hash "$GRANDSTREAM_SRC" "$GRANDSTREAM_V2_SHA256" Grandstream.py
  ...
  check_hash "$SCAN_STATUS_SRC" "$SCAN_STATUS_SHA256" paloEndpointScanStatus.class.php
...
verify(){
  ...
  local df
  check_payload
  check_hash "$GRANDSTREAM_DST" "$GRANDSTREAM_V2_SHA256" live-Grandstream.py
  ...
  check_hash "$SCAN_STATUS_DST" "$SCAN_STATUS_SHA256" live-paloEndpointScanStatus
...
install_all(){
  ...
  local installing=1
  trap 'rc=$?; if [ "$installing" -eq 1 ]; then echo "[Endpoint Configuration 1.0.1-rc4] ERROR: restaurando runtime desde backup RC4" >&2; restore_runtime || true; apachectl -t >/dev/null 2>&1 && systemctl reload httpd || true; fi; exit "$rc"' ERR
...
RC4 instala la imagen dorada auditada de Ceiba:
- Grandstream V2
- Avaya J129
- Extension / Registration
- seguridad administrativa y vault cifrado

Instala y verifica los mismos hashes auditados en Ceiba. NO ejecuta discovery, NO asigna extensiones y NO configura/reinicia teléfonos.
EOF
}
