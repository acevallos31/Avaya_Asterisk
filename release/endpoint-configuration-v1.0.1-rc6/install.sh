...
DISCOVERY_SRC="${GOLDEN_PAYLOAD}/detect_endpoints"
...
DISCOVERY_DST='/usr/share/issabel/privileged/detect_endpoints'
...
CLEAN_BOOTSTRAP_MARKER='RC6-DISCOVERY-BOOTSTRAP'
...
make_defaults(){
  ...
  [ -n "$h" ] || h=localhost
  f="$(mktemp /tmp/endpoint-config-rc6-db.XXXXXX.cnf)"; chmod 0600 "$f"
  {
    echo '[client]'
    printf 'user="%s"\n' "$(escape_mysql "$u")"
    printf 'password="%s"\n' "$(escape_mysql "$p")"
...
bootstrap_schema_with_dba() (
  ...
  dba_cnf="$(mktemp /tmp/endpoint-config-rc6-dba.XXXXXX.cnf)"
  chmod 0600 "$dba_cnf"
...
VALUES
  (NULL, 'INSTALL_PRIVILEGE_TEST', 'PENDING', 'rc6-installer',
   '00000000-0000-0000-0000-000000000006', NOW());
UPDATE endpoint_credential_event
SET result='PASS'
WHERE correlation_id='00000000-0000-0000-0000-000000000006';
...
check_payload(){
  local f
  for f in     "$GRANDSTREAM_SRC" "$CATALOG_SQL" "$AVAYA_SRC" "$AVAYA_J129_TPL_SRC" "$AVAYA_GLOBAL_TPL_SRC" "$AVAYA_HTTP_SRC"     "$ENDPOINTCONFIG_BIN_SRC" "$BASE_ENDPOINT_SRC" "$SCAN_STATUS_SRC" "$ENDPOINTS_LIB_SRC" "$STANDARD_MANAGER_SRC" "$DISCOVERY_SRC"     "$SCHEMA_SQL" "$KEY_INSTALLER" "$VAULT_SRC" "$CLI_SRC" "$INDEX_SRC" "$SUMMARY_SRC" "$SUMMARY_LANG_SRC" "$REPORT_SRC" "$JS_SRC"; do
    need_file "$f"
  ...
  PYTHONDONTWRITEBYTECODE=1 python3 -m py_compile "$DISCOVERY_SRC"
  ...
  grep -Fq "_probe_grandstream_model_from_asterisk" "$DISCOVERY_SRC" || die 'discovery Asterisk model fallback missing'
  grep -Fq "_save_registered_model_fallback" "$DISCOVERY_SRC" || die 'discovery model persistence missing'
  grep -Fq "sip show peer " "$DISCOVERY_SRC" || die 'discovery SIP peer Useragent probe missing'
  log 'RC6-DISCOVERY-BOOTSTRAP-PAYLOAD-PASS'
...
check_grandstream(){
  ...
  if cmp -s "$GRANDSTREAM_DST" "$GRANDSTREAM_SRC"; then
    log "Grandstream runtime RC6 discovery-bootstrap presente sha256=$sha"
    return 0
  fi
  case "$sha" in
...
preflight(){
  ...
  rpm -q issabel-endpointconfig2-5.0.0-1.el8.noarch >/dev/null 2>&1 ||
    die 'RC6 requiere issabel-endpointconfig2-5.0.0-1.el8.noarch, la misma base auditada en Ceiba'
...
install_base_runtime(){
  ...
  install -o root -g root -m 0755 "$DISCOVERY_SRC" "$DISCOVERY_DST"
  ...
  PYTHONDONTWRITEBYTECODE=1 python3 -m py_compile "$DISCOVERY_DST"
...
verify(){
  ...
  cmp -s "$DISCOVERY_SRC" "$DISCOVERY_DST" || die 'detect_endpoints discovery-bootstrap runtime mismatch'
  grep -Fq "_probe_grandstream_model_from_asterisk" "$DISCOVERY_DST" || die 'live discovery Asterisk model fallback missing'
  grep -Fq "_save_registered_model_fallback" "$DISCOVERY_DST" || die 'live discovery model persistence missing'
  ...
  log 'RC6-DISCOVERY-BOOTSTRAP-PASS'
  log 'discovery_useragent_model=YES'
  log 'discovery_ip_extension_reconcile=YES'
  log 'VERIFY-PASS'
...
install_all(){
  ...
  local installing=1
  trap 'rc=$?; if [ "$installing" -eq 1 ]; then echo "[Endpoint Configuration 1.0.1-rc6] ERROR: restaurando runtime desde backup RC6" >&2; restore_runtime || true; apachectl -t >/dev/null 2>&1 && systemctl reload httpd || true; fi; exit "$rc"' ERR
...
restore_runtime(){
  local state dst rel
  [ -f "$MANIFEST" ] || die 'no existe manifest RC6 de backup'
...
RC6 instala la imagen dorada auditada de Ceiba más el bootstrap para una central limpia:
- Grandstream V2
- Avaya J129
- Extension / Registration
- seguridad administrativa y vault cifrado

Conserva el runtime auditado de Ceiba. RC6 corrige el flujo real de la lupa: detect_endpoints obtiene el modelo Grandstream desde el Useragent de un peer ya registrado y el cierre del scan reconcilia IP->extensión. NO ejecuta discovery automáticamente y NO configura/reinicia teléfonos.
EOF
}
