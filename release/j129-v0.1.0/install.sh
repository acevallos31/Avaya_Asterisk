...
HTTP_SRC="${PAYLOAD}/etc/httpd/conf.d/avaya-j129-provisioning.conf"
...
preflight(){
  ...
  python3 -c "import ast; ast.parse(open('$VENDOR_SRC', encoding='utf-8').read(), filename='$VENDOR_SRC')"
  local df mf model bad_prefix multiaccount
  ...
  multiaccount="$(scalar "$df" "SELECT COUNT(*) FROM (SELECT ea.id_endpoint FROM endpoint_account ea JOIN endpoint e ON e.id=ea.id_endpoint JOIN manufacturer mf ON mf.id=e.id_manufacturer LEFT JOIN model m ON m.id=e.id_model WHERE mf.name='Avaya' AND m.name='J129' GROUP BY ea.id_endpoint HAVING COUNT(*)>1) q;")"
  [ "$multiaccount" -eq 0 ] || die 'existe al menos un J129 con mas de una cuenta; v0.1.0 no migra multicuenta automaticamente'
...
verify(){
  ...
  python3 -c "import ast; ast.parse(open('$VENDOR_DST', encoding='utf-8').read(), filename='$VENDOR_DST')"
  apachectl -t
