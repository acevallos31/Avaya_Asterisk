...
def sip_status(ep):
    return "✅ REGISTERED" if ep.get("state") == "CONFIGURED_REGISTERED" else "❌ NOT REGISTERED"


def live_status(ep):
    state = ep.get("live_state", "UNKNOWN")
    return {
        "LIVE_CONFIRMED": "✅ LIVE + MAC MATCH",
        "LIVE_IP_MAC_MISMATCH": "🚨 MAC MISMATCH",
        "LIVE_IP_ONLY": "⚠️ LIVE IP / MAC UNKNOWN",
        "UNREACHABLE_OR_FILTERED": "⚠️ NO ICMP",
    }.get(state, "❓ UNKNOWN")
...
def markdown_lines(report):
    ...
    lines = [
        ...
        "> Auditoría de diagnóstico **sin cambios de configuración**. La prueba envía un ping corto para refrescar presencia/neighbor y nunca muestra contraseñas SIP.", "",
        "## Estado general", "", "| Indicador | Resultado |", "|---|---:|",
        "| 🔎 Avaya detectados | **{}** |".format(n(s, "total")),
        "| ⚙️ Configurados | **{}** |".format(n(s, "configured")),
        "| ✅ Registrados | **{}** |".format(n(s, "registered")),
        "| ❌ Configurados sin registro | **{}** |".format(n(s, "not_registered")),
        "| 💤 Solo detectados | **{}** |".format(n(s, "detected_only")),
        "| 🟢 Presencia IP + MAC confirmada | **{}** |".format(n(s, "live_confirmed")),
        "| 🟡 IP responde pero MAC no verificable | **{}** |".format(n(s, "live_ip_only")),
        "| 🚨 MAC viva distinta a Endpoint Configurator | **{}** |".format(n(s, "live_mac_mismatch")),
        "| ⚠️ Sin respuesta ICMP / filtrado | **{}** |".format(n(s, "live_unreachable")),
        "| 🔐 Credenciales coinciden | **{}** |".format(n(s, "credentials_match")),
        "| 🚨 Credenciales distintas | **{}** |".format(n(s, "credentials_mismatch")),
        "| ❓ Credenciales no verificables | **{}** |".format(n(s, "credentials_unknown")),
        "| 📥 Archivo solicitado | **{}** |".format(n(s, "config_requested")),
        "| ⚠️ Sin evidencia de descarga | **{}** |".format(n(s, "config_not_requested")), "",
    ]
    if issues:
        lines += ["## 🚨 Requieren atención", ""]
        for ep in issues:
            lines.append("- **Ext. {}** · {} · {} · MAC `{}` · última IP `{}`".format(safe(ep.get("account")), safe(ep.get("state")), live_status(ep), safe(ep.get("mac")), safe(ep.get("last_ip"))))
        lines.append("")
    else:
    ...
    lines += ["", "## Detalle de configuración, presencia y registro", ""]
    for ep in configured:
        lines += [
            "<details>",
            "<summary><strong>Ext. {}</strong> · {} · {} · {}</summary>".format(safe(ep.get("account")), safe(ep.get("model")), live_status(ep), sip_status(ep)), "",
            "- MAC guardada en Endpoint Configurator: `{}`".format(safe(ep.get("mac"))),
            "- Última IP detectada: `{}`".format(safe(ep.get("last_ip"))),
            "- Último scan: `{}`".format(safe(ep.get("last_scanned"))),
            "- Última modificación: `{}`".format(safe(ep.get("last_modified"))),
            "- Ping actual: **{}**".format(safe(ep.get("ping"))),
            "- MAC observada en neighbor/ARP: `{}`".format(safe(ep.get("live_mac"))),
            "- MAC viva vs MAC DB: **{}**".format(safe(ep.get("live_mac_match"))),
            "- Estado neighbor: `{}`".format(safe(ep.get("neighbor_state"))),
            "- Presencia actual: **{}**".format(safe(ep.get("live_state"))),
            "- IP SIP registrada: `{}`".format(safe(ep.get("sip_ip"))),
            "- User-Agent: `{}`".format(safe(ep.get("useragent"))),
            "- Usuario provisioning vs Asterisk: **{}**".format(safe(ep.get("credential_user", "UNKNOWN"))),
            "- Secret provisioning vs Asterisk: **{}**".format(safe(ep.get("credential_secret", "UNKNOWN"))),
            "- Solicitud de configuración: **{}**".format(safe(ep.get("config_request", "NO"))),
            "- IP que solicitó archivo: `{}`".format(safe(ep.get("request_ip"))),
            "- HTTP: `{}`".format(safe(ep.get("request_http"))),
            "- Última solicitud encontrada: `{}`".format(safe(ep.get("request_time"))),
            "- Estado final SIP/provisioning: **{}**".format(safe(ep.get("state"))), "", "</details>", "",
        ]

    lines += ["## Avaya detectados sin extensión", "", "| Modelo | MAC DB | Última IP | Último scan | Presencia actual | MAC viva | Coincidencia |", "|---|---|---|---|---|---|---|"]
    for ep in detected:
        lines.append("| {} | `{}` | `{}` | `{}` | {} | `{}` | **{}** |".format(safe(ep.get("model")), safe(ep.get("mac")), safe(ep.get("last_ip")), safe(ep.get("last_scanned")), live_status(ep), safe(ep.get("live_mac")), safe(ep.get("live_mac_match"))))
    ...
    lines += ["", "---", "Interpretación: `LIVE_CONFIRMED` exige que la IP responda y que la MAC observada coincida con la MAC de Endpoint Configurator. `UNREACHABLE_OR_FILTERED` no demuestra que el teléfono esté apagado: ICMP puede estar filtrado. En redes enrutadas, la MAC de neighbor puede no ser la del teléfono.", "", "Archivos descargables: TXT (evidencia sanitizada), JSON (datos estructurados) y PDF (reporte legible)."]
    return lines
...
def report_lines(report):
    ...
    lines = [
        ...
        "  Credenciales MATCH/MISMATCH/UNKNOWN: {}/{}/{}".format(s.get("credentials_match", "0"), s.get("credentials_mismatch", "0"), s.get("credentials_unknown", "0")),
        "  Archivo solicitado / sin evidencia: {}/{}".format(s.get("config_requested", "0"), s.get("config_not_requested", "0")), "", "TELEFONOS CONFIGURADOS",
    ]
    ...
    for ep in configured:
        ...
        lines += ["", "Ext. {} | {} | {}".format(safe(ep.get("account")), safe(ep.get("model")), safe(ep.get("state"))),
    ...
    for ep in detected:
        lines.append("  {} | {} | IP {} | live {} | live MAC {} | match {} | scan {}".format(safe(ep.get("model")), safe(ep.get("mac")), safe(ep.get("last_ip")), safe(ep.get("live_state")), safe(ep.get("live_mac")), safe(ep.get("live_mac_match")), safe(ep.get("last_scanned"))))
    lines += ["", "FABRICANTES"]
    for mf in report.get("manufacturers", []):
        lines.append("  {}: {} endpoint(s)".format(safe(mf.get("name")), safe(mf.get("endpoints"))))
    lines += ["", "NOTAS", "  LIVE_CONFIRMED requiere respuesta IP y coincidencia de MAC observada con Endpoint Configurator.", "  UNREACHABLE_OR_FILTERED no demuestra que el telefono este apagado; ICMP puede estar filtrado.", "  En redes enrutadas la MAC de neighbor puede corresponder al siguiente salto."]
    return lines
...
def write_pdf(report, path):
    ...
    for idx, lines in enumerate(pages):
        ...
        for line in lines:
            ...
            stream += ["({}) Tj".format(pdf_escape(latin)), "T*"]
        stream.append("ET")
        stream_text = "\n".join(stream) + "\n"
    ...
    for num, obj in enumerate(objects, 1):
        offsets.append(len(body)); body.extend("{} 0 obj\n{}\nendobj\n".format(num, obj).encode("latin-1", "replace"))
...
def main():
    ...
    parser.add_argument("--summary", dest="summary_path")
    args = parser.parse_args()
    report = parse_log(args.log)
    write_json(report, args.json_path); write_pdf(report, args.pdf_path)
