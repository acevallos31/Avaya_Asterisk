...
ENC_USER="$(python3 -c 'import os,urllib.parse; print(urllib.parse.quote_plus(os.environ.get("GRANDSTREAM_HTTP_USERNAME","admin")))')"
ENC_PASS="$(python3 -c 'import os,urllib.parse; print(urllib.parse.quote_plus(os.environ["GRANDSTREAM_HTTP_PASSWORD"]))')"
...
LOGIN_SHA="$(sha256sum "$BODY" | awk '{print $1}')"
...
{
  echo '=== G10-11 GRANDSTREAM GXP1625 EXACT HTTP SESSION AUDIT ==='
  echo "phone_ip=${PHONE_IP}"
  echo "firmware_prog=${FIRMWARE}"
  echo 'payload_shape=EXACT-G10-09-USERNAME-PLUS-PASSWORD'
  ...
  echo "login_body_sha256=${LOGIN_SHA}"
  ...
  echo
  echo "followup_model_http=${FOLLOW_CODE}"
  echo "followup_model_content_type=${FOLLOW_CT}"
  echo "followup_model_bytes=${FOLLOW_BYTES}"
  echo "followup_model_sha256=${FOLLOW_SHA}"
  echo
  echo 'G10-11-PASS'
} | tee "$REPORT"
